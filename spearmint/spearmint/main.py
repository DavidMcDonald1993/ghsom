...
def write_config_file(exp_name, location):

    data = {"language": "PYTHON",
            "main-file": "spearmint_ghsom.py",
            "experiment-name": "{}_exp".format(exp_name),
            "likelihood": "GAUSSIAN",
            "variables": {
            "e_sg": {"type": "FLOAT","size": 1, "min": 0.5, "max": 1.1}}}
...
def get_options():
    ...
    #                   type="string", default="karate_config.json")
    parser.add_option("--name", "-n", dest="name",
                  help="Name of experiment.",
                  type="string", default="karate")
    parser.add_option("--labels", "-l", dest="labels",
    ...
    expt_file = os.path.join(expt_dir, '{}_config.json'.format(commandline_kwargs.name))

    if not os.path.isfile(expt_file):
        ##create a config file if one does not already exist
        print '{} does not exist, creating it'.format(expt_file)
        write_config_file(commandline_kwargs.name, expt_file)
    ...
    options["config"]  = '{}_config.json'.format(commandline_kwargs.name)
    ...
    options['name'] = commandline_kwargs.name
    options['labels'] = commandline_kwargs.labels
...
def main():
    ...
    while not complete:
        ...
        for resource_name, resource in resources.iteritems():
            ...
            if resource.numComplete(jobs) >= MAX_JOBS:
            	print 'completed max jobs'
                complete = True
                continue
            ...
            while resource.acceptingJobs(jobs):
                ...
                process_id = resource.attemptDispatch(experiment_name, suggested_job, db_address, expt_dir, 'embedded_{}.gml'.format(options['name']), options['labels'], options['name'])
                ...
                # if num_complete_jobs(jobs) >= 2:
                	
                #     print 'completed 5 jobs'
                ...
                #             continue
                #         if job['values']['main'][0] < min_fitness:
		              #       min_fitness = job['values']['main'][0]
		              #       params = job['params']
   	                
   	            #     print 'best params: {}'.format(params)

   	            #     e_sg = params['e_sg']['values'][0]
   	            #     print 'best e_sg: {}'.format(e_sg)
   	            #     save_obj(e_sg, 'best_e_sg_{}'.format(options['name']))
   	            #     print 'DONE'
                #     return
    ...
    print 'broken out of while'
    best_e_sg = get_best_e_sg(jobs)
    path = 'C:\Miniconda3\Jupyter\GHSOM_simplex_dsd\parameter_tests_density_spearmint'
    save_obj(best_e_sg, 'best_e_sg_{}'.format(options['name']), path=path)
    
    print 'DONE'

def get_best_e_sg(jobs):
    min_fitness = 1
    params = {}
    for job in jobs:
        if not job['status'] == 'complete':
