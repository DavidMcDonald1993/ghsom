...
def initialise_network(ID, X, starting_nodes=3):
    ...
    return network, V, signal_frequencies
...
def train_network(X, network, V, signal_frequencies, num_epochs, eta_0, precomputed_sigmas):
...
def compute_probabilities(network, V, relative_signal_frequencies):
    ...
    volumes = np.array([np.mean(np.array([distances[network.nodes()[i]][neighbour]
                                       for neighbour in neighbours[i]])) ** d for i in range(num_neurons)])
    
    print "VOLUMES"
    print volumes
    
    print "SIGNAL FREQUENCIES"
    print relative_signal_frequencies
    
    return relative_signal_frequencies / volumes * sum(volumes)
...
def ghsom(ID, named_X, num_iter, eta, sigma, e_0, e_sg, e_en, q):
    ...
    network, V, signal_frequencies = initialise_network(ID, X)
    
    #precompute sigmas
    precomputed_sigmas = precompute_sigmas(sigma, num_iter)
    
    #train for lamda epochs
    V, signal_frequencies = train_network(X, network, V, signal_frequencies, num_iter, eta, precomputed_sigmas)
    ...
    while MQE > e_sg * e_0 and num_deleted_neurons < MAX_DELETED_NEURONS:
        ...
        #expand network
        V = expand_network(ID, named_X, network, V, signal_frequencies, error_unit)
